...
drop trigger if exists import_candidates_dedup_before_insert on public.import_candidates;
...
returns table (transaction_count bigint, import_candidate_count bigint, dedup_key_count bigint, estimated_bytes bigint)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  return query select
    ...
    ((select count(*) from public.transactions t join public.trackers tr on tr.id = t.tracker_id where tr.user_id = current_user_id) * 1100 +
     ...
     (select count(*) from public.import_dedup_keys where user_id = current_user_id) * 110)::bigint;
end;
$$;
...
begin
  if current_user_id is null then raise exception 'Not authenticated'; end if;
  if p_retention_days < 30 or p_retention_days > 3650 then raise exception 'Retention must be between 30 days and 10 years'; end if;
  delete from public.import_candidates
  where user_id = current_user_id and status in ('imported', 'excluded', 'removed')
